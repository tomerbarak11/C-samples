...
static Vector* maxHeapify (Vector* _vec,int _i,LessThanComparator _pfLess);
static void swap(Vector* _vec,int index1,int index2);
static void* maxValueOf2(void* _a,void* _b,LessThanComparator _pfLess);
static void* maxValueOf3(void* _a,void* _b,void* _c,LessThanComparator _pfLess);
...
Heap* HeapBuild(Vector* _vec, LessThanComparator _pfLess)
{
	...
	{
		_vec=maxHeapify(_vec,i,_pfLess);
	}
...
static Vector* maxHeapify (Vector* _vec,int _i,LessThanComparator _pfLess)
{
	...
	max= maxValueOf3(father,left,right,_pfLess);
...
void* HeapExtract(Heap* _heap)
{
	...
	if(	_heap->m_magicNumber==123456789)
	{
		...
		{
			_heap->m_vec=maxHeapify(_heap->m_vec,i,_heap->fp);
		}
		return max;
	}
	return NULL;
