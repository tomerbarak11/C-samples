...
void checkCurrClient(List* clients,fd_set* master_socket,fd_set* temp,int* activity)
{
	...
	ListItr ItrClient2, ItrEnd2,tempItr2;
	...
	while(!ListItr_Equals(ItrClient,ItrEnd))
	{
		if(*activity!=0)
		{
			...
			if(FD_ISSET(client_sock,temp))
			{
				...
				/*while*/
				ItrClient2 = ListItr_Begin(clients);
				ItrEnd2 = ListItr_End(clients);
				while(!ListItr_Equals(ItrClient2,ItrEnd2))
				{
					client_sock2 =*(int*)ListItr_Get(ItrClient2);
					if(!FD_ISSET(client_sock,temp))
					{printf("4444444444444444444444444444444444\n");
						sent_bytes = send(client_sock2,buffer, strlen(buffer),0);
						...
						{ 
							...
							close(client_sock2);
							tempItr2=ItrClient2;
							ItrClient2=ListItr_Prev(ItrClient2);
							FD_CLR(client_sock2,master_socket);
							ListItr_Remove(tempItr2);
							(*activity)--;		
						}
					}
					ItrClient2 = ListItr_Next(ItrClient2);
				}	
			}
			ItrClient = ListItr_Next(ItrClient);
		}
